    ...
    if (cartItemIndex === -1) {
      try {
        const res = await fetch(
          `https://crudcrud.com/api/0c6bbaa42d1b4a059a520dfb26cf55f1/cartItem${userEmail}`,
          {
            method: 'POST',
            body: JSON.stringify({
              title: product.title,
      ...
      try {
        await fetch(
          `https://crudcrud.com/api/0c6bbaa42d1b4a059a520dfb26cf55f1/cartItem${userEmail}/${updatedItem[cartItemIndex]._id}`,
          {
            method: 'PUT',
            body: JSON.stringify({
              title: product.title,
